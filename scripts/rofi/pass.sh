...
# shows a list of all password files and saves the selected entry
...
if [[ $typeit -eq 0 ]]; then
    pass show -c "$entry" | head -n1 2>/dev/null
else
    ...
    passw=$(pass show "$entry" | head -n1)
    uname=$(pass show "$entry" | tail -n1)
